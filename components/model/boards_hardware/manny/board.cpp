...
#if !CONFIG_ROCKET_AP_ALLOW_OPEN_DEVELOPMENT
static_assert(sizeof(CONFIG_ROCKET_AP_PASSWORD) >= 9,
              "Operational builds require CONFIG_ROCKET_AP_PASSWORD with at least 8 characters");
#endif
...
MannyBoard::MannyBoard()
    : is_initialized(false),
      shared_service_mutex(xSemaphoreCreateRecursiveMutex()),
...
MannyBoard::~MannyBoard() {
    if (shared_service_mutex != nullptr) {
        vSemaphoreDelete(shared_service_mutex);
        shared_service_mutex = nullptr;
    }
}
...
/**
 * @brief RAII guard over the board shared-service recursive mutex.
 *
 * Falls through without locking when the mutex could not be created, so a
 * failed allocation degrades to the previous unsynchronized behavior instead
 * of deadlocking the caller.
 */
class SharedServiceLock {
public:
    explicit SharedServiceLock(SemaphoreHandle_t mutex) : _mutex(mutex) {
        if (_mutex != nullptr) {
    ...
    SharedServiceLock(const SharedServiceLock&) = delete;
    SharedServiceLock& operator=(const SharedServiceLock&) = delete;
...
bool MannyBoard::initNvs() {
    SharedServiceLock lock(shared_service_mutex);
...
#if CONFIG_ROCKET_AP_ALLOW_OPEN_DEVELOPMENT
        LOG_WARNING(TAG, "Starting explicitly enabled development-only open SoftAP");
#else
        LOG_ERROR(TAG, "Refusing open SoftAP in an operational build");
        return false;
#endif
    }
