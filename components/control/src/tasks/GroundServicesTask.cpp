...
{
    // Every failure below clears `running` so taskFunction() returns
    // immediately and BaseTask still calls onTaskStop(), which releases the
    // SoftAP reference and the mutexes. Returning while `running` is true
    // would leave a task spinning forever holding those resources.
    ...
    // Never abort here: a persistently unwritable NVS would turn every boot
    // into a panic reboot. Ground Services simply does not come up, and the
    // flight tasks fall back to the compiled defaults.
    const esp_err_t cfg_err = runtime_config_init(_board);
    if (cfg_err != ESP_OK) {
        LOG_ERROR(TAG, "Failed to initialize runtime config: %s", esp_err_to_name(cfg_err));
        running = false;
        return;
    }
...
void GroundServicesTask::registerHandlers()
{
    auto add = [this](const char *uri, httpd_method_t method, esp_err_t (*handler)(httpd_req_t *)) {
        ...
        h.handler = handler;
        h.user_ctx = this;
        // Registration only fails when max_uri_handlers is exhausted or the
        // arguments are malformed. Log it instead of aborting: losing one
        // endpoint must not panic-reboot the flight computer.
        const esp_err_t reg_err = httpd_register_uri_handler(_server, &h);
        if (reg_err != ESP_OK) {
            LOG_ERROR(TAG, "Failed to register handler for %s: %s", uri, esp_err_to_name(reg_err));
        }
    };
